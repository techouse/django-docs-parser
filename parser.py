...
DJANGO_VERSIONS = ("5.0", "4.2", "4.1", "4.0", "3.2", "3.1", "3.0", "2.2", "2.1", "2.0", "1.11", "1.10", "1.8")
...
def parse():
    ...
    for version in DJANGO_VERSIONS:
        with ZipFile(
            BytesIO(
                requests.get(DJANGO_DOCS_DOWNLOAD_URL.format(version=version)).content
            )
        ) as zipfile:
            with zipfile.open("genindex.html") as genindex:
                ...
                for index, table in enumerate(
                    index_soup.find_all("table", class_="genindextable")
                ):
                    if index < 2:
                    ...
                    for a in table.find_all("a"):
                        ...
                        if file.startswith("releases/") or any(
                            id.startswith(pattern) for pattern in EXCLUDED_IDS
                        ):
                            continue
                        ...
                        with zipfile.open(file) as html:
                            try:
                                ...
                                entry_data = {
                                    ...
                                    "title": "",
                                    "permalink": urljoin(
                                        DJANGO_DOCS_URL.format(version=version),
                                        href.replace(".html", "/"),
                                    ),
                                    "categories": [],
                                    "content": "",
                                ...
                                if parent.name == "dl" and topic.name == "dt":
                                    ...
                                    if topic.find(class_="property"):
                                        entry_data["categories"].append(
                                            topic.find(class_="property").get_text()
                                        )
